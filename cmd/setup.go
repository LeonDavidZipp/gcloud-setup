...
// runSetup executes the complete GCloud project and GitHub repository setup process.
// It validates prerequisites, collects configuration interactively, and runs all setup steps.
...
// saveConfig writes the configuration to .env.gcloud.local and ensures it is gitignored.
...
// ensureGitignore adds the specified entry to .gitignore if not already present.
...
// loadConfig reads configuration from viper and returns a populated Config struct.
...
// In non-interactive mode, it returns the default value or existing config.
...
// promptConfirm asks the user for a yes/no confirmation and returns the result.
...
// interactiveConfig guides the user through configuration collection with smart defaults.
...
// parseGitRemote extracts the organization and repository name from a git remote URL.
// Supports SSH format (git@github.com:org/repo.git) and HTTPS format (https://github.com/org/repo.git).
...
// listBillingAccounts retrieves all open billing accounts from GCP.
...
// checkGH verifies that the GitHub CLI is installed and authenticated.
func checkGH() error {
...
// enableAPIs enables all required GCP APIs for the project.
...
// configureGitHub sets up GitHub repository secrets, variables, and deployment environments.
